...
from algorithms.meme import MeMeTrainer

class ParallelMeMeTrainer(ParallelTrainer, MeMeTrainer):
    identifier = "parallelMeme"
    ...
    def master_train(self):
        ...
        while not stopMainLoop:
            ...
            actions, TOM_out = self.get_TOM_and_actions(num_actions = self.N_worker)
            ...
            for _ in range(self.N_worker):
                ...
                #store result to update network after worker-loop
                result_buffer.append((self.action, self.reward))
            ...
            self.loss = self.calculateLoss(result_buffer, TOM_out)
